	...
	<div class="content inner">
		<section class="panel__div panel-message__div clearfix">
			...
			<div class="pull-left">
				<div class="msg-list">
				    @foreach($link as $k=>$v)
					<a class="ep active" href="{{ $v->id }}">{{ $v->name }} {{$v->cartname}}</a>
					@endforeach
				</div>
			...
			<div class="message-box pull-right">
				<div class="head-div clearfix posr">
					<div class="title">{{ $notices->name}}{{ $notices->cartname }}</div>
					<div class="time pull-right">发布时间：{{ $notices->created_at }}</div>
				</div>
				<div class="html-code">
					<p></p>
					<p style="text-indent: 2em">{!! $notices->left !!}</p>
